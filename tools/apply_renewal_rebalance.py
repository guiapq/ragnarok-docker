#!/usr/bin/env python3
# =============================================================================
# RAGNAROGUE – Renewal / WoE TE & Patch 2024 Rebalance Applicator
#
# Aplica integralmente o rebalanceamento de Transclasses e Ninjas
# ("A Caminho das Classes 4" / Patch 2024 & Class Improvement):
#   1. size_fix.txt: Modificadores de tamanho por arma (Knuckles 100/100/75, etc.)
#   2. import/skill_cast_db.txt: Castings, Delays e Cooldowns do Patch 2024 & Renewal
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. GERAÇÃO DO IMPORT/SKILL_CAST_DB.TXT (Patch 2024 & Renewal Cast/Delay/Cooldown)
# ─────────────────────────────────────────────────────────────────────────────
def update_skill_cast():
    ...
    for sid, line in patch_2024_overrides.items():
        re_skills[sid] = line
    ...
    output_lines = [
        "// ============================================================================\n",
        "// RAGNAROGUE – WoE TE & Patch 2024 Rebalance Skill Times (Cast, Delay, Cooldown)\n",
    ...
    for group_name, prefs in prefixes.items():
        ...
        for sid, name in snames.items():
            ...
            if any(name.startswith(p) for p in prefs) and sid in re_skills:
                used_sids.add(sid)
                tag = " [Patch 2024]" if sid in patch_2024_overrides else ""
    ...
    print(f"  [skill_cast_db.txt] Gerado com {total_count} habilidades (incluindo as 31 do Patch 2024).")
...
def update_skill_db():
    ...
    print(f"  [pre-re/skill_db.txt] Atualizado com {updated} habilidades modificadas para o padrão Patch 2024.")
...
def update_skill_require_db():
    ...
    print(f"  [pre-re/skill_require_db.txt] Atualizado com {updated} requisitos de SP, catalisadores e esferas Patch 2024.")
...
def update_skill_damage():
    ...
    content = """// ============================================================================
// RAGNAROGUE – WoE TE & Patch 2024 Skill Damage Adjustments
...
if __name__ == "__main__":
    print("=== Aplicando Rebalanceamento Transclasses & Ninjas (Patch 2024 / WoE TE) ===")
    update_size_fix()
    update_skill_cast()
    update_skill_db()
